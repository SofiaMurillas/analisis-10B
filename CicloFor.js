...
for(var i=1; i<=5; i++){ //no siempre el ciclo for debe empezar en cero
    nombre=prompt("Ingrese el nombre número "+i)//el mensaje es más específico para que el usuario sepa en qué número de 
    //registro está
    arregloNombres.push(nombre); //.push() es el método que permite insertar elementos al final del arreglo
}

//6
for(var i=0; i<arregloNombres.length; i++){ //i inicia en 0 porque las posiciones de los arreglos comienzan en cero y 
    //va hasta arregloNombres.length que retorna el tamaño del arreglo (cantidad de elementos dentro)
    console.log("El nombre en la posicion "+ i +" es "+ arregloNombres[i]) //imprime el valor de cada posición. i va 
    //aumentando, por tanto, va avanzando en posiciones. arregloNombres[i] accede al valor en la posición
}
